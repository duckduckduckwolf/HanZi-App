...
/**
 * A full-screen view of everything we know about a word: the big character,
 * what the card tests you on, and the full (cleaned) dictionary entry — every
 * reading and sense, plus a per-character breakdown for compounds.
 *
 * This is a normal in-flow screen (not a fixed overlay) so it scrolls with the
 * app's main scroll container. iOS Safari does not reliably touch-scroll the
 * inner content of a `position: fixed` overlay, which is why an earlier
 * bottom-sheet version felt "cut off" on iPhone.
 */
export default function WordDetailView({ hanzi, card, onBack }: Props) {
  ...
  return (
    <div className="screen detail-screen" data-testid="word-detail">
